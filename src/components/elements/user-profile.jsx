          ...
          <div className="user-profile">
            <div className="media">
              <div className="media-left">
                  {props.isLoading && !props.profilePictureLargeUrl ? (
                    ...
                    <Link to={`/${props.username}`} className="userpic userpic-large">
                      <img src={props.profilePictureLargeUrl} width="75" height="75" className="media-object img-rounded"/>
                    </Link>
                  )}
                  {/* <button className="btn btn-xs btn-block follow">Follow</button> */}
                  ...
                  {props.isPrivate === '1' && !props.amISubscribedToUser ? (
                    props.hasRequestBeenSent ? (
                      props.userView.isSubscribing ? 'Revoking...' : <a className="btn btn-xs btn-block follow" onClick={this.handleRevokeSentRequest}>Revoke request</a>
                    ) : (
                      props.userView.isSubscribing ? 'Requesting...' : <a className="btn btn-xs btn-block follow" onClick={this.handleSendSubRequest}>Request a subscription</a>
                    )
                  ) : (
                    props.amISubscribedToUser ? (
                      props.userView.isSubscribing ? 'Unsubscribing...' : <a className="btn btn-xs btn-block follow" onClick={this.handleUnsubscribe}>Unsubscribe</a>
                    ) : (
                      props.userView.isSubscribing ? 'Subscribing...' : <a className="btn btn-xs btn-block follow" onClick={this.handleSubscribe}>Follow</a>
                    )
                  )}
              ...
              <div className="media-body">
                  <span className="pull-right"><i className="fa fa-chevron-down"></i></span>
                  <h3 className="media-heading profile-username">
                      {props.isLoading && !props.screenName ? (
                        ...
                        <div>
                          <div className="profile-displayname">
                            <Link to={`/${props.username}`}>{props.screenName}</Link>
                          </div>
